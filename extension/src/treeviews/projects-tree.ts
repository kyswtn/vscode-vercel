...
export class ProjectsTreeView implements vscode.Disposable {
  ...
  private refreshProjects() {
    this.linkedProjectsState.reloadLocalProjectsAndLinkThem()
  }

  private async openProjectOnVercel(project: LinkedProject) {
    await vscode.env.openExternal(vscode.Uri.parse(project.remote.url))
  }

  private async copyProjectUrl(project: LinkedProject) {
    await vscode.env.clipboard.writeText(project.remote.url)
  }
